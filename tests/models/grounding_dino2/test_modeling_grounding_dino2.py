...
if is_torch_available():
    ...
    from transformers import GroundingDino2Config, GroundingDino2ForObjectDetection, GroundingDino2Model
...
class GroundingDino2ModelTester:
    def __init__(
        self,
        parent,
        batch_size=4,
    ...
    def create_and_check_model(self, config, pixel_values, pixel_mask, input_ids, labels):
        model = GroundingDino2Model(config=config)
    ...
    def create_and_check_object_detection_head_model(self, config, pixel_values, pixel_mask, input_ids, labels):
        model = GroundingDino2ForObjectDetection(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class GroundingDino2ModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (GroundingDino2Model, GroundingDino2ForObjectDetection) if is_torch_available() else ()
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False):
        ...
        if return_labels:
            if model_class.__name__ == "GroundingDino2ForObjectDetection":
                labels = []
    ...
    def setUp(self):
        self.model_tester = GroundingDino2ModelTester(self)
    ...
    def test_two_stage_training(self):
        model_class = GroundingDino2ForObjectDetection
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.return_dict = True
    ...
    def test_tied_weights_keys(self):
        ...
        for model_class in self.all_model_classes:
            ...
            # GroundingDino2 when sharing weights also uses the shared ones in GroundingDino2Decoder
            # Therefore, differently from DeformableDetr, we expect the group lens to be 2
            # one for self.bbox_embed in GroundingDino2ForObejectDetection and another one
            # in the decoder
...
@require_vision
@slow
class GroundingDino2ModelIntegrationTests(unittest.TestCase):
    @cached_property
    def default_processor(self):
        return AutoProcessor.from_pretrained("superb-ai/grounding-dino2-tiny") if is_vision_available() else None

    def test_inference_object_detection_head(self):
        model = GroundingDino2ForObjectDetection.from_pretrained("superb-ai/grounding-dino2-tiny").to(torch_device)
    ...
    def test_grounding_dino2_loss(self):
        ...
        model = GroundingDino2ForObjectDetection.from_pretrained(
            "superb-ai/grounding-dino2-tiny", auxiliary_loss=True, class_loss_reduction="sum"
        )
