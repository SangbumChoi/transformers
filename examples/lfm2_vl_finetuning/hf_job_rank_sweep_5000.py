...
SEED = 42
N_TRAIN = 5000
CONFIGS = [(f, r) for f in ["vision_all", "language_all", "all_both"] for r in [1, 4, 16, 64]]
N_TEST = 100
MAX_STEPS = 2000
BATCH = 8
LR = 2e-4
...
def main():
    ...
    print(
        f"train={len(train)} fixed test={len(test)}; rank sweep over {CONFIGS}; max_steps={MAX_STEPS} batch={BATCH}",
        flush=True,
    )
    ...
    for family, rank in CONFIGS:
        ...
        targets = target_names(model, family)
        ...
        sft = SFTConfig(
            output_dir=f"/tmp/r_{family}_{rank}",
            max_steps=MAX_STEPS,
        ...
        trainer.train()
        trainer.model.eval()
        sub = random.Random(0).sample(train, 100)
        res = {
            "config": family,
            "rank": rank,
            ...
            "median_rel_update": round(median_rel_update(trainer.model), 4),
        }
    ...
    print("\n==== RANK SWEEP @ 5000 SUMMARY ====")
    print(f"{'config':14s}{'rank':>5}{'params':>11}{'train':>7}{'test':>7}{'med_dW':>9}")
    for r in sorted(results, key=lambda x: (x["config"], x["rank"])):
        print(
            f"{r['config']:14s}{r['rank']:5d}{r['trainable_params']:11d}{r['train_acc_sub']:7.0%}{r['test_acc']:7.0%}{r['median_rel_update']:9.3f}"
        )
