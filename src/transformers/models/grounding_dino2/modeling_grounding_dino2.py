...
from .configuration_grounding_dino2 import GroundingDino2Config
...
_CONFIG_FOR_DOC = "GroundingDino2Config"
_CHECKPOINT_FOR_DOC = "superb-ai/grounding-dino2-tiny"


@dataclass
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoDecoderOutput with GroundingDino->GroundingDino2
class GroundingDino2DecoderOutput(ModelOutput):
    """
    Base class for outputs of the GroundingDino2Decoder. This class adds two attributes to
...
@dataclass
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoEncoderOutput with GroundingDino->GroundingDino2
class GroundingDino2EncoderOutput(ModelOutput):
    """
    Base class for outputs of the GroundingDino2Encoder. This class extends BaseModelOutput, due to:
...
@dataclass
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoObjectDetectionOutput with GroundingDino->GroundingDino2,GroundingDinoProcessor->GroundingDinoProcessor
class GroundingDino2ObjectDetectionOutput(ModelOutput):
    """
    Output type of [`GroundingDino2ForObjectDetection`].
...
# Copied from transformers.models.detr.modeling_detr.DetrFrozenBatchNorm2d with Detr->GroundingDino2
class GroundingDino2FrozenBatchNorm2d(nn.Module):
    """
...
# Copied from transformers.models.detr.modeling_detr.replace_batch_norm with Detr->GroundingDino2
def replace_batch_norm(model):
    r"""
    Recursively replace all `torch.nn.BatchNorm2d` with `GroundingDino2FrozenBatchNorm2d`.
    ...
    for name, module in model.named_children():
        if isinstance(module, nn.BatchNorm2d):
            new_module = GroundingDino2FrozenBatchNorm2d(module.num_features)
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoConvEncoder with GroundingDino->GroundingDino2
class GroundingDino2ConvEncoder(nn.Module):
    """
    Convolutional backbone, using either the AutoBackbone API or one from the timm library.

    nn.BatchNorm2d layers are replaced by GroundingDino2FrozenBatchNorm2d as defined above.
...
# Copied from transformers.models.detr.modeling_detr.DetrConvModel with Detr->GroundingDino2
class GroundingDino2ConvModel(nn.Module):
    """
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoSinePositionEmbedding with GroundingDino->GroundingDino2
class GroundingDino2SinePositionEmbedding(nn.Module):
    """
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoLearnedPositionEmbedding with GroundingDino->GroundingDino2
class GroundingDino2LearnedPositionEmbedding(nn.Module):
    """
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.build_position_encoding with GroundingDino->GroundingDino2
def build_position_encoding(config):
    if config.position_embedding_type == "sine":
        position_embedding = GroundingDino2SinePositionEmbedding(config)
    elif config.position_embedding_type == "learned":
        position_embedding = GroundingDino2LearnedPositionEmbedding(config)
    else:
...
class GroundingDino2MultiscaleDeformableAttention(nn.Module):
    ...
    def __init__(self, config: GroundingDino2Config, num_heads: int, n_points: int):
        super().__init__()
        ...
        if not ((dim_per_head & (dim_per_head - 1) == 0) and dim_per_head != 0):
            warnings.warn(
                "You'd better set embed_dim (d_model) in GroundingDino2MultiscaleDeformableAttention to make the"
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoTextEnhancerLayer with GroundingDino->GroundingDino2
class GroundingDino2TextEnhancerLayer(nn.Module):
    """Vanilla Transformer with text embeddings as input"""

    def __init__(self, config):
        super().__init__()
        self.self_attn = GroundingDino2MultiheadAttention(
    ...
    def forward(
        ...
        the text encoder (AutoModel based on text_config) within GroundingDino2EncoderLayer
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoBiMultiHeadAttention with GroundingDino->GroundingDino2
class GroundingDino2BiMultiHeadAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Copied from transformers.models.beit.modeling_beit.BeitDropPath with Beit->GroundingDino2
class GroundingDino2DropPath(nn.Module):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoFusionLayer with GroundingDino->GroundingDino2
class GroundingDino2FusionLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class GroundingDino2DeformableLayer(nn.Module):
    def __init__(self, config: GroundingDino2Config):
        super().__init__()
        self.embed_dim = config.d_model
        self.self_attn = GroundingDino2MultiscaleDeformableAttention(
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoEncoderLayer with GroundingDino->GroundingDino2
class GroundingDino2EncoderLayer(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()

        self.d_model = config.d_model

        self.text_enhancer_layer = GroundingDino2TextEnhancerLayer(config)
        self.fusion_layer = GroundingDino2FusionLayer(config)
        self.deformable_layer = GroundingDino2DeformableLayer(config)
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoMultiheadAttention with GroundingDino->GroundingDino2
class GroundingDino2MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in GroundingDino2Model forward() function)
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoDecoderLayer with GroundingDino->GroundingDino2
class GroundingDino2DecoderLayer(nn.Module):
    def __init__(self, config: GroundingDino2Config):
        super().__init__()
        self.embed_dim = config.d_model

        # self-attention
        self.self_attn = GroundingDino2MultiheadAttention(config, num_attention_heads=config.decoder_attention_heads)
        ...
        self.encoder_attn_text = GroundingDino2MultiheadAttention(
        ...
        # cross-attention
        self.encoder_attn = GroundingDino2MultiscaleDeformableAttention(
            config,
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoPreTrainedModel with GroundingDino->GroundingDino2
class GroundingDino2PreTrainedModel(PreTrainedModel):
    config_class = GroundingDino2Config
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, GroundingDino2LearnedPositionEmbedding):
            nn.init.uniform_(module.row_embeddings.weight)
            nn.init.uniform_(module.column_embeddings.weight)
        elif isinstance(module, GroundingDino2MultiscaleDeformableAttention):
            module._reset_parameters()
        elif isinstance(module, GroundingDino2BiMultiHeadAttention):
        ...
        elif isinstance(module, (GroundingDino2EncoderLayer, GroundingDino2DecoderLayer)):
        ...
        elif isinstance(module, GroundingDino2MLPPredictionHead):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, GroundingDino2Decoder):
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoEncoder with GroundingDino->GroundingDino2
class GroundingDino2Encoder(GroundingDino2PreTrainedModel):
    """
    Transformer encoder consisting of *config.encoder_layers* deformable attention layers. Each layer is a
    [`GroundingDino2EncoderLayer`].
    ...
    def __init__(self, config: GroundingDino2Config):
        ...
        self.dropout = config.dropout
        self.layers = nn.ModuleList([GroundingDino2EncoderLayer(config) for _ in range(config.encoder_layers)])
    ...
    def forward(
        ...
        Args:
            ...
            semantic_features (`torch.FloatTensor` of shape `(batch_size, semantic_seq_len, hidden_size)`):
                Semantic features that are passed to the encoder.
            output_attentions (`bool`, *optional*):
                Whether or not to return the attentions tensors of all attention layers. See `attentions` under
                returned tensors for more detail.
            output_hidden_states (`bool`, *optional*):
...
class GroundingDino2Decoder(GroundingDino2PreTrainedModel):
    ...
    def __init__(self, config: GroundingDino2Config):
        ...
        self.layers = nn.ModuleList([GroundingDino2DecoderLayer(config) for _ in range(config.decoder_layers)])
        self.reference_points_head = GroundingDino2MLPPredictionHead(
    ...
    def forward(
        ...
        return GroundingDino2DecoderOutput(
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoModel with GroundingDino->GroundingDino2,IDEA-Research/grounding-dino-tiny->superb-ai/grounding-dino2-tiny
class GroundingDino2Model(GroundingDino2PreTrainedModel):
    def __init__(self, config: GroundingDino2Config):
        super().__init__(config)

        # Create backbone + positional encoding
        backbone = GroundingDino2ConvEncoder(config)
        position_embeddings = build_position_encoding(config)
        self.backbone = GroundingDino2ConvModel(backbone, position_embeddings)
        ...
        # Monkey patch custom CLIP backbone (deprecated)
        ...
        self.encoder = GroundingDino2Encoder(config)
        self.decoder = GroundingDino2Decoder(config)
        ...
        if config.two_stage:
            ...
            if (
                ...
                self.encoder_output_bbox_embed = GroundingDino2MLPPredictionHead(
                    input_dim=config.d_model, hidden_dim=config.d_model, output_dim=4, num_layers=3
                )

            self.encoder_output_class_embed = GroundingDino2ContrastiveEmbedding(config)
        else:
    ...
    @replace_return_docstrings(output_type=GroundingDino2ModelOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        pixel_values: Tensor,
        ...
        >>> processor = AutoProcessor.from_pretrained("superb-ai/grounding-dino2-tiny")
        >>> model = AutoModel.from_pretrained("superb-ai/grounding-dino2-tiny")
        ...
        # If the user passed a tuple for encoder_outputs, we wrap it in a GroundingDino2EncoderOutput when return_dict=True
        ...
        return GroundingDino2ModelOutput(
...
class GroundingDino2MLPPredictionHead(nn.Module):
    """
...
# Copied from transformers.models.deformable_detr.modeling_deformable_detr.DeformableDetrHungarianMatcher with DeformableDetr->GroundingDino2
class GroundingDino2HungarianMatcher(nn.Module):
    """
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoLoss with GroundingDino->GroundingDino2
class GroundingDino2Loss(nn.Module):
    """
    This class computes the losses for `GroundingDino2ForObjectDetection`. The process happens in two steps: 1) we
    compute hungarian assignment between ground truth boxes and the outputs of the model 2) we supervise each pair of
    matched ground-truth / prediction (supervise class and box).

    Args:
        matcher (`GroundingDino2HungarianMatcher`):
...
class GroundingDino2ForObjectDetection(GroundingDino2PreTrainedModel):
    ...
    def __init__(self, config: GroundingDino2Config):
        ...
        self.model = GroundingDino2Model(config)
        _class_embed = GroundingDino2ContrastiveEmbedding(config)

        if config.decoder_bbox_embed_share:
            _bbox_embed = GroundingDino2MLPPredictionHead(
                input_dim=config.d_model, hidden_dim=config.d_model, output_dim=4, num_layers=3
            )
    ...
    @add_start_docstrings_to_model_forward(GROUNDING_DINO_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=GroundingDino2ObjectDetectionOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        ...
        input_semantics: torch.FloatTensor = None,
        pixel_mask: Optional[torch.BoolTensor] = None,
        encoder_outputs: Optional[Union[GroundingDino2EncoderOutput, Tuple]] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        ...
        ```python
        >>> from transformers import AutoProcessor, GroundingDino2ForObjectDetection
        >>> from PIL import Image
        >>> import requests
        ...
        >>> processor = AutoProcessor.from_pretrained("superb-ai/grounding-dino2-tiny")
        >>> model = GroundingDino2ForObjectDetection.from_pretrained("superb-ai/grounding-dino2-tiny")
        ...
        dict_outputs = GroundingDino2ObjectDetectionOutput(
            loss=loss,
            loss_dict=loss_dict,
            logits=logits,
